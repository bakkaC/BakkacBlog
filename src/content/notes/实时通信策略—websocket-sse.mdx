...
WebSocket：需要手动实现重连和保活机制

```js
let socket;
let reconnectAttempts = 0;
const maxReconnectAttempts = 10;
const baseInterval = 5000;   // 初始重连间隔 5s
const pingInterval = 30000;  // 心跳间隔 30s
let pingTimer;
...
function connectWebSocket() {
  socket = new WebSocket("ws://your-server-url/chat");
  ...
  socket.onclose = () => {
    clearInterval(pingTimer);   // 停止心跳
    if (reconnectAttempts < maxReconnectAttempts) {
      reconnectAttempts++;
      // 指数退避：base × 2^attempts，上限 30s
      const delay = Math.min(
        baseInterval * Math.pow(2, reconnectAttempts),
        30000
      );
...
1. 启动连接时，要启动心跳，每隔轮询 ping 一次，避免因为长时间不通信而断开连接，浪费资源
2. 关闭连接时，要停止心跳，按照指数退避的方式重试连接:
    假设你有 10,000 个客户端连接到同一个服务端，导致服务端宕机了：
	如果所有客户端都用固定 5 秒重连，服务端刚恢复就被 10,000 个请求同时砸中，又被压垮——这就是**雪崩效应**（Thundering Herd）。恢复 → 压垮 → 恢复 → 压垮，死循环。
	 
    **第一层：为什么需要退避？** 固定 5 秒重连会导致雪崩——10,000 个客户端同时重试，服务端刚恢复就被压垮，形成死循环。
    
	**第二层：为什么不用固定长间隔（比如直接 30 秒）？** 如果只是网络抖动 3 秒就恢复了，你却等 30 秒才重连，用户体验太差。第一次重连应该快——10 秒就够了，也许只是短暂抖动。
	
	**第三层：为什么是指数而不是线性？** 这是关键。看上图的对比：
	
	- **固定 5s**：2 分钟内 24 次重连，请求**均匀分布**——服务端始终承受 2,000 req/s 的恒定压力，永远没有恢复的窗口
	- **指数退避**：2 分钟内只有 5 次重连，而且**间隔越来越大**（10s → 20s → 30s → 30s）——服务端在两次重连波之间获得了越来越长的喘息时间
	
	线性退避（5s, 10s, 15s, 20s...）虽然也在减速，但减速太慢——前 2 分钟内仍然会有约 7 次重连，而且间隔增长太慢。指数退避的**衰减速度刚好**：前几次快速试探（也许只是短暂故障），后续越来越慢（大概是持久故障，别急着重试）。
