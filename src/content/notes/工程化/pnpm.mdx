...
1. 依赖包作为普通文件，可能被修改，那包语义会被破坏，工具无法正常分析，行为和官方可能不一致，不利于合作
	...
	2. 项目状态无法从 `package.json` 和 lockfile 完整恢复。
		例如：为了修一个 bug，直接改了：`node_modules/lodash/index.js`，本地运行正常。但同事执行 `npm install` 后，你的修改消失了。你的机器：正常，同事机器：有 bug，CI：有 bug
2. 可能造成幽灵依赖，`"dependencies": {}` ，数组里没有声明，但是却可以 `import` ，这些依赖通常是一些包的子依赖，依赖和声明的不一致会导致问题：也就是没有声明的依赖都是不可靠的，因为依赖是依靠工具和他人维护的包进行引入自动化管理的，只有声明的依赖被工具保证存在，例如：
	1. 一个依赖的版本升级，子依赖不被保证继续存在
	2. 换个包管理器后，安装布局变了，lodash 不再能被解析。
